...
function validateform() {
    ...
    if (uname == null || uname == "") {
        document.getElementById("status").innerText = "please enter a new user name";
        document.getElementById("UserName").focus()
        document.getElementById("UserName").value = "";
        return false;
    }
    else if (uname.length < 4) {
        document.getElementById("status").innerText = "user name should be atleast 4 characters long";
        document.getElementById("UserName").focus()
        document.getElementById("UserName").value = "";
        return false;
    }
    if (pwd == null || pwd == "") {
        document.getElementById("status").value = "please enter a new password";
        document.getElementById("Password").focus()
        return false;
    }
    if (pwd.length < 6 || pwd.length > 18) {
        document.getElementById("status").innerText = "password should be between 6 to 18 characters long";
        document.getElementById("Password").focus()
        document.getElementById("Password").value = "";
        return false;
    }

    getUserDetails(); getUserKey();
...
getUserDetails = function GetUserDetails() {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost/ZKP/Account/Login.aspx/GetUserDetails",
        ...
        success: function (data) {
            ...
            if (data == undefined || data == '')
                document.getElementById("status").value = "Login is not successful";
            user.p = data.d.P;
            user.g = data.d.G;
...
function Encrypt(userkey, data) {
    ...
    var encGx = CryptoJS.AES.encrypt(CryptoJS.enc.Utf8.parse(data), key, {
...
function ProverState0(encryptedData, userkey) {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost/ZKP/Account/Login.aspx/VerifierState0",
        ...
        error: function (response) {
            document.getElementById("status").innerText = "Login is not successful";
            reset();
        }
    });
}
...
function ProverState1() {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost/ZKP/Account/Login.aspx/VerifierState1",
        ...
        success: function (response) {
            if (response.d) {
                document.getElementById("status").innerText = "Login is successful";
                reset();
            }
            else {
                document.getElementById("status").innerText = "Login is not successful";
        ...
        error: function (response) {
            document.getElementById("status").innerText = response.responseText;
            reset();
